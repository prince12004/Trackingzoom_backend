...
/**
 * Populates realistic PLACEHOLDER catalog/marketing content so the storefront
 * renders fully populated instead of empty. Uses stock photography as a
 * temporary stand-in — replace every image here with real product/brand
 * photography from Admin before going live. Does NOT create any fake
 * customer reviews, expert reviews, or fake people — those must be real.
 */
async function upsertCategory(input) {
    const slug = (0, slug_1.toSlug)(input.name);
    return Category_1.Category.findOneAndUpdate({ slug }, { ...input, slug, status: 'active' }, { upsert: true, new: true });
}
async function main() {
    await (0, db_1.connectDB)();
    console.log('[demo] Connected to database');
    // ---- Brand ----
    const brand = await Brand_1.Brand.findOneAndUpdate({ slug: 'trackingzoom' }, {
        name: 'TrackingZoom',
        slug: 'trackingzoom',
    ...
    const demoProducts = [
        {
            ...
            description: '<p>The TZ Wireless GPS Tracker Mini gives you real-time location tracking without any wiring. Magnetic mount, long battery life, and instant mobile alerts make it ideal for cars, trailers and rented vehicles.</p>',
            regularPrice: 3499,
            salePrice: 2799,
        ...
        },
        {
            ...
            description: '<p>Built for fleet owners and daily drivers who want permanent tracking. Hardwired to the vehicle battery for uninterrupted power, with optional remote engine immobilization.</p>',
            regularPrice: 4999,
            salePrice: 3999,
        ...
        },
        {
            ...
            description: '<p>A wide-angle, waterproof reverse parking camera with night vision — get a clear rear view on your dashboard display every time you back up.</p>',
            regularPrice: 1599,
            salePrice: 1199,
            image: 'https://images.unsplash.com/photo-1600661653561-629509216228?w=800&q=80',
    ...
    for (const p of demoProducts) {
        const slug = (0, slug_1.toSlug)(p.name);
        let product = await Product_1.Product.findOne({ sku: p.sku });
        const payload = {
            name: p.name,
            slug,
            sku: p.sku,
        ...
        if (product) {
            ...
            product = await Product_1.Product.create(payload);
    ...
    for (const b of banners) {
        await Banner_1.Banner.findOneAndUpdate({ title: b.title }, { ...b, status: 'active', startDate: new Date('2026-01-01'), endDate: new Date('2027-01-01') }, { upsert: true });
    }
    console.log('[demo] Banners ready');
    // ---- Offer ----
    await Offer_1.Offer.findOneAndUpdate({ title: 'Festive Sale on GPS Trackers' }, {
    ...
    for (const f of faqs) {
        await FAQ_1.FAQ.findOneAndUpdate({ question: f.question }, { ...f, status: 'active' }, { upsert: true });
    }
    console.log('[demo] FAQs ready');
    // ---- Blog ----
    const blogCategory = await BlogCategory_1.BlogCategory.findOneAndUpdate({ slug: 'vehicle-safety' }, { name: 'Vehicle Safety', slug: 'vehicle-safety', status: 'active' }, { upsert: true, new: true });
    const blogs = [
        {
        ...
        },
        {
            ...
            content: '<p>Choosing between a wired and wireless GPS tracker depends on your use case.</p><h3>Wireless Trackers</h3><p>Battery-powered, magnetic mount, no installation needed — ideal for temporary tracking or rental vehicles.</p><h3>Wired Trackers</h3><p>Hardwired to the vehicle battery for continuous power and advanced features like remote immobilization — best for daily drivers and fleets.</p>',
        },
    ];
    for (const b of blogs) {
        const slug = (0, slug_1.toSlug)(b.title);
        await Blog_1.Blog.findOneAndUpdate({ slug }, {
            ...b,
            slug,
