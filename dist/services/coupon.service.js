...
async function validateAndComputeCoupon(code, userId, lines, subtotal) {
    ...
    if (now < coupon.startDate || now > coupon.expiryDate) {
        throw ApiError_1.ApiError.badRequest('This coupon has expired or is not yet active');
    }
    if (subtotal < coupon.minCartAmount) {
        throw ApiError_1.ApiError.badRequest(`Minimum cart amount of ₹${coupon.minCartAmount} required for this coupon`);
    }
    if (coupon.usageLimit && coupon.usedCount >= coupon.usageLimit) {
        throw ApiError_1.ApiError.badRequest('This coupon has reached its usage limit');
    }
    if (coupon.perUserLimit) {
        const userUsageCount = await Order_1.Order.countDocuments({
            user: userId,
            coupon: coupon._id,
    ...
    if (coupon.firstOrderOnly) {
        const anyOrder = await Order_1.Order.exists({ user: userId, paymentStatus: { $ne: 'failed' } });
        if (anyOrder)
            throw ApiError_1.ApiError.badRequest('This coupon is valid only on your first order');
    }
    ...
    if (coupon.applicableProducts.length > 0 || coupon.applicableCategories.length > 0) {
        ...
        if (applicableTotal === 0) {
            throw ApiError_1.ApiError.badRequest('This coupon is not applicable to the items in your cart');
        }
    }
