...
const crud = (0, crudFactory_1.createCrudController)(Offer_1.Offer, { entityName: 'Offer', defaultSort: { displayOrder: 1 } });
router.get('/', (0, catchAsync_1.catchAsync)(async (_req, res) => {
    const now = new Date();
    const items = await Offer_1.Offer.find({ status: 'active', startDate: { $lte: now }, endDate: { $gte: now } })
