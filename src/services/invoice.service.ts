...
export async function streamInvoicePdf(order: IOrder, res: Response): Promise<void> {
  ...
  if (settings.business.gstNumber) doc.text(`GSTIN: ${settings.business.gstNumber}`);
  doc.moveDown();
  ...
  doc.text(
    `${order.billingAddress.addressLine1}${order.billingAddress.addressLine2 ? ', ' + order.billingAddress.addressLine2 : ''}`
  );
  ...
  if (order.codCharge) summaryLine('COD Charge', order.codCharge);
  ...
  if (order.ewayBillNumber) doc.text(`E-Way Bill: ${order.ewayBillNumber}`, 50, y + 28);
